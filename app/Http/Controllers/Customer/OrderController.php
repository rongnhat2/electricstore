...
class OrderController extends Controller
{
    ...
    public function __construct(Product $product, Customer $customer, Order $order, OrderDetail $order_detail){
    ...
    public function checkout(Request $request){
        ...
        if ($is_user) { 
            ...
            foreach ($item as $key => $item_id) {
                ...
                if (count($product) > 0) {
                    $product = $product[0];
                    $discount_product   = $product->prices * $product->discount / 100;
