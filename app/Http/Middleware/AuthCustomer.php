            ...
            if ($token) {
                ...
                if ($user) {
                    ...
                    if ($user->status) {
                        ...
                        $request->session()->forget('_token_');
                        Cookie::queue(Cookie::forget('_token_'));
                        return redirect()->route('customer.view.index')->with('error', 'Tài khoản đã bị khóa!');  
                    }
                }else{
                    $request->session()->forget('_token_');
                    Cookie::queue(Cookie::forget('_token_'));
                    return redirect()->route('customer.view.index')->with('success', 'Tài khoản không tồn tại!');  
                }
            }else{
                return $next($request);
            }
