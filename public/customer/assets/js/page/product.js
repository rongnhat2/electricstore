const View = {
    Description: {
        render(data){
            ...
            discount != 0 ? $(".product-images-wrapper").prepend(`
                <span class="onsale">-
                    <span class="woocommerce-Price-amount amount">
                        <span class="woocommerce-Price-currencySymbol">đ </span>${sell_prices}
                    </span>
                </span>
            `) : "";
            prices = "";
            discount != 0 ? prices += `<del class="font-size-20 ml-2 text-gray-6">${View.formatNumber(data.prices)} đ</del> ` : "";
            prices += `<ins class="font-size-36 text-decoration-none">${real_prices} đ</ins> `;
            ...
            $(".category-name").attr("href", `/category?tag=${data.category_id}`)
            ...
            $(".data-warehouse").html(data.warehouse_quatity ? data.warehouse_quatity + " sản phẩm" : `<span class="text-red">Hết hàng</span>`)
            $(".product-detail").html(data.detail)
            data.metadata.split(",").map((value, key) => {
                $(".metadata-list").append(`
                    <tr>
                        <th class="px-4 px-xl-5 border-top-0">${value.split(":")[0]}</th>
                        <td class="border-top-0">${value.split(":")[1]}</td>
                    </tr>
                `)
            })
        }
...
(() => {
    ...
    function getProduct(){
        ...
        Api.Product.GetOne(View.URL.get("id"))
            .done(res => {
                View.Images.render_list(res.data.data_product[0].images);
                View.Description.render(res.data.data_product[0]);
