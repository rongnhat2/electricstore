            ...
            $(".cart-table-wrapper tbody")
                .append(`<tr data-id="${data.id}"
                            ...
                            <td class="d-none d-md-table-cell">
                                <a href="/product/${data.slug}?id=${data.id}" class="cart-image-item"><img class="img-fluid max-width-100 p-1 border border-color-1" src="${image}" alt="Image Description"></a>
                            </td>
                            <td data-title="Product">
                                <a href="/product/${data.slug}?id=${data.id}" class="text-gray-90">${data.name}</a>
                            </td>
                            <td data-title="Price">
                                <span class="">${ViewIndex.Config.formatPrices(data.prices)} đ</span>
                            </td>
                            ...
                            <td data-title="Price">
                                ...
                                ${discount == 0 ? "" : `<span class="discount-price data-total-discount">-${ViewIndex.Config.formatPrices(discount)} đ</span>`}
                            </td>
                            <td data-title="Total">
                                ${discount == 0 ? "" : `<del class="data-total-prices-defaul">${ViewIndex.Config.formatPrices(data.prices*quantity)} đ</del>`}
        ...
        if (cart.length > 0) {
            cart.map((v, k) => {
                Api.Product.GetOneItem(v)
                    .done(res => {
                        if (res.status == 200) View.Item.render(res.data[0], k)
                    })
                    .fail(err => {  })
                    .always(() => { });
            })
